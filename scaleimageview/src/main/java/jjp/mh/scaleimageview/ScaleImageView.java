...
/**
 * Created by mhjjp 2018年03月01日16:23:39
 *
 *  具有缩放功能,解决了与ViewPager的滑动冲突
 */
public class ScaleImageView extends AppCompatImageView implements ViewTreeObserver.OnGlobalLayoutListener, ScaleGestureDetector.OnScaleGestureListener, View.OnTouchListener {
    ...
    public void Reset(){
        ...
        int drawableWidth = dWidth;
        int drawableHeight = dHeight;
        ...
        //postDelayed(new AutoScaleRunnable(mInitScale, 0, 0), 16);
        //isAutoScale = true;
    }
    ...
    private void checkBorderAndCenterWhenScale() {
        ...
        if (zoomedListener != null) {
            rect = getMatrixRectF();
            //Log.i(TAG, String.format("current width: %s, height: %s, left: %s, top: %s", rect.width(), rect.height(), rect.left, rect.top));
            zoomedListener.onZooming(rect);
        }
    ...
    public void onScaleEnd(ScaleGestureDetector detector) {
        // 自动加载大图
        if (urlLarge != null && !urlLarge.isEmpty() && !hasLarge) {
            //ImageUtils.LoadImg(urlLarge, this);
            hasLarge = true;
        }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        //获取原来的中心点的X值
        float orginCenterX = getWidth()/2;
        //获取滑动时刻 中心点的X值
        float nowCenterx = rectF.centerX();
        //放大后 中心点距离orginCenter的X点最大距离
        float moveX = Math.abs(rectF.width()/2-getWidth()/2);

        // 通过X1和X2的差，获得X轴上面滑动方向。 分别处理左右方向
        //getParent().requestDisallowInterceptTouchEvent(true) 师父容器ViewPager不拦截事件
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if(X1 - X2 > 0) {
                    if (nowCenterx > orginCenterX - moveX +10) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                //向左滑动图片   nowCenter 大于 orginCenter-moveX+10    现在中心点的X值 大于 最小中心点X值+10单位距离  父容器不去拦截事件
                //事件传递给ImageView  最后 return true  消费点改事件，这个时候就做到了只是滑动了ImageView
                if(X1 - X2 > 0) {
                    if (nowCenterx > orginCenterX - moveX +10 ) {
                        getParent().requestDisallowInterceptTouchEvent(true);
                    }
    ...
    public OnImageZoomedListener zoomedListener;

    public void setZoomedListener(OnImageZoomedListener listener) {
        this.zoomedListener = listener;
    }

    public interface OnImageZoomedListener {
        void onZoomBegin();

        void onZooming(RectF rect);

        void onZoomEnd();

        void onMoved(RectF rect);
    }
